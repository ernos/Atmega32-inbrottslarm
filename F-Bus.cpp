...
extern void usart_transmit(char *);
...
unsigned char InitFrames[] ={
	...
	/* sms central comviq: +46707990001 
		0x07 - length (bytes)
		0x91 - international
		0xF0 - padding*/ 
...
*/
	...
	unsigned char RecipientNo[] =	{0x0B, 0x91, 0x64, 0x37, 0x56, 0x86, 0x88, 0xF1, 0x00, 0x00, 0x00, 0x00};
...
ref: http://www.embedtronics.com/nokia/fbus.html#part2
smsc:+46707990001 
recp:+46736568881
//
*/																				//F=padding
	//unsigned char RecipientNo[] =	{0x02 ,0x0b ,0x69 , 0x74 , 0x07 , 0x73 , 0x21 ,0x36 , 0xF5};
...
int put_uart_char(char c, FILE *fd)
{
	...
	ISR(USART_RXC_vect)
	{
		chr=UDR; //read the recived char
			...
			if ((ch[3] == 0x00) && (ch[4] == 0x0c) && (ch[5] == 0x1e))
			{
				ReciveMode=0;
			}
		...
		else
		{
			...
			if(ptr++==(size))
			{
				...
				{
					//printf("Decoding SMS....\n");
					memcpy(frameBuffer, FBusFrame, size);
	...
	unsigned char decodeSMS(unsigned char *src,unsigned char *out,unsigned char size)
	{
		...
		//frameBuffer[SMSstart] = out[0];
		//iframeBuffer = SMSstart;
		while( 1)
		{
			//iframeBuffer++;
			...
			w >>= (8 - i);
			out[j] = c | w;
			//frameBuffer[iframeBuffer] = out[j];
			//putchar(out[j]);
			i++;
			j++;
			if(i==8)
...
exit:
	...
	{ 
		...
		{
			putchar (FBusFrame [j]);
			frameBuffer[iframeBuffer] = FBusFrame[j];
			iframeBuffer++;
		}
	...
	fdevopen(&put_uart_char, 0);
		...
		{
			putc(GetSwHwCmd[i], stdout);
			//usart_transmit(GetSwHwCmd[i]);
		}
		sei();
	}
	...
	void ParseSMS(unsigned char *msg)
	{
		...
		strcpy(MytestString, (const char *)msg);
		msglength1 = strlen((const char *)msg);
		
		if(strcmp ("testing",(const char *)msg)== 0)
		{
			PinNumber = msg[4] - '0' ;
			if((PinNumber > 0) && (PinNumber < 3))
			{
				// check the command is ON or OFF
				if(strncmp (" on",(const char *)msg+5,3)== 0)
				{
					/*Turn on*/
...
//						__start_timout ();
		...
		else if(strncmp ("send temp.",(const char *)msg,10) == 0)
		{
